...
const Sidebar = () => {
  ...
  return (
    <div className={`${isOpen ? 'right-0' : '-right-full'} w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] xl:max-v-[30vw]
   transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
      <div className='flex items-center justify-between py-6 border-b'>
        <div className='uppercase text-sm font-semibold'>Shopping Bag ({itemAmount})</div>
        <div onClick={handleClose} className='flex justify-center items-center cursor-pointer w-8 h-8'>
     ...
     border-b overflow-x-hidden'>
      ...
      <div className='flex flex-col gap-y-3 py-4 mt-4'>
        <div onClick={() => clearCart()} className='flex w-full justify-between items-center p-2'>
          <div className='uppercase font-semibold'><span className='mr-2'>Total:</span> $ {parseFloat(total).toFixed(2)}</div>
          <div className='py-4 cursor-pointer bg-red-500 text-white w-12 h-12 flex justify-center items-center text-xl'>
            <FiTrash2 />
          </div>
        </div>
